...
void Lexer<ImplTraits>::displayRecognitionError( ANTLR_UINT8** , ExceptionBaseType* ex)
{
    ...
    else
    {
		...
		// err_stream << "(";
		// err_stream << "file \'" << ex->get_streamName().c_str() << "\' near line ";
    }
...
// 	err_stream << ex->get_line() << ": ";
...

	{
		...
		if	(width >= 1)
		{
			if	(isprint(ex->get_c()))
			{
				err_stream << "'" << ((char)ex->get_c()) << "'";
			}
			else
			{
				char tmp[128];
				sprintf( tmp, "near char(%#02X) (perhaps you used a non-ASCII letter?)", ex->get_c() );
				err_stream << tmp;
			}
			// err_stream << "\t";
    ...
    err_stream << "; error near line " << ex->get_line();
